...
export default function MultipleSelector(props: multipleSelectorProps) {
  // functions to select and unselect items
  function select(item: multipleSelectorModel) {
    const selected = [...props.selected, item]; // concatenate the selected items
  ...
  function deselect(item: multipleSelectorModel) {
    const nonSelected = [...props.nonSelected, item]; // concatenate the non-selected items
